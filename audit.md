...
**Measured:** 7 August 2026, against `6dc72c9` ("chore: rewrite some parts"), which is the whole
working tree apart from this file. Every number here was taken off the files rather than carried
forward.

**The range is four commits and it rewrote nearly every unit**, so almost nothing in this file was
carried across: every unit in all four steps moved, twelve figures and five quizzes arrived, two
step 2 units were retitled, and the last pass's uncommitted work is now committed underneath three
further commits. Read an unchanged row as re-measured rather than skipped.
...
### Table 1a. Step 0, "Start here" (2 units, 785 words)

**Every row in this table is one defect**, and it is the largest thing this pass found: step 0 was
rewritten around a third unit, a three-flag board, that the registry never learned about. The Java,
the HTML, the board, the flags and half the locale keys are all in the tree.

| Where | Topic | Status | Effort | Remarks and proposed fix |
|---|---|:--:|:--:|---|
| `start here/how this kata works` | The page promises a page that does not exist | ⚠ | ●○○ | **1.** Three blocks point at a third unit in this step. `welcome.lead.2`: "The last page of this step checks all three for you." `welcome.how-workshops-work.1`: "Steps 0, 1 and 2 each have one." `welcome.house-rules.5`: "the page after it is this step's board." **The step renders two units**, and the sidebar shows two. All three are true of the tree and false of the app, so the fix is item 4 rather than a rewrite; if that is not taken, all three sentences come out, in both languages. |
| | `set-your-assistant` renders as nothing | ⚠ | ●○○ | **2.** `welcome.html:44` leaves a `<div data-figure="set-your-assistant">` and `index.tsx` registers `code-check`, `hint-check` and `legend` for this unit and not that one, so `StepContent` renders the segment as an empty fragment. `SetYourAssistant.tsx` exists, it is a one-move `TaskCard`, and its four keys are in **both** locale files. Everything but one line of the registry is there, and what the student sees is the paragraph "Set the assistant to the one you actually use" followed by a gap. Fix: one entry in `index.tsx`. |
| `start here/the backend` | The unit has nothing on it | ⚠ | ●○○ | **3.** Same failure, both ways round. `backend.html` leaves a `<div data-figure="intro-loop">` that the registry does not fill, and the registry still supplies `'code-check': <CodeCheck code={finishCode} />` for a marker the HTML no longer carries. So `IntroLoop` never draws and the answer box never renders: the page is **235 words of prose, one `<pre>` and nothing else**, where the last pass measured a code box. `backend.code-blocks.2` then closes on "The next page puts three of those runs on a board", which is item 1's missing page again. |
| `front/src/steps/step0/index.tsx` | The intro's third unit is never registered | ○ | ●●○ | **4.** `units/workshop.html`, `FlagBoard.tsx`, `flags.ts`, `IntroLoop.tsx` and `SetYourAssistant.tsx` are all in `front/src/steps/step0/` and **nothing imports any of them**. The Java half is finished too: `kata/step0/java/pom.xml` carries the `intro`, `ready` and `pick` profiles and `src/test` carries an `IT` for each. What is missing is a `units:` entry, four `inlineFigures` lines and one import. Fix: wire it. It is the cheapest unfinished thing in the repository. |
| | The locale keys landed half way | ◐ | ●●○ | **5.** `flags.ts` asks for `flag.run.*`, `flag.ready.*` and `flag.pick.*`, `FlagBoard` asks for `flags.panel.*` and `IntroLoop` for `loop.*`: **none of those keys exists in either locale file.** The unit's own prose keys (`workshop.lead.*`, `workshop.two-candidates.*`) and `workshop.title` have no Dutch either, and they are the only untranslated prose in the course (Table 3). Meanwhile `en.json` and `nl.json` still carry the dead `flag.panel.*` block, ten keys for the answer box item 3 unwired, and a fully written and fully translated second quiz question, `quiz.agent-says-it-found-it.*`, that `quiz.ts` does not export. Fix: write the missing keys, delete the two dead blocks or wire the question. |
...
| — | Boundaries cut token cost | ○ | ●○○ | **10.** The architecture argument is on the page and the money argument is not. This is the only place the course connected layout to the bill and it is still absent, across a range that added a whole unit about what things cost. Fix: one sentence beside the `tokens` link. |
...
| | Over-commenting and under-logging | ○ | ●○○ | **12.** Went with `quality` and has **no second home anywhere in the course**: neither term appears in any unit, in either language, and a grep over all 25 files returns nothing. It was two sentences and it is hygiene an agent gets wrong by default. Fix: put it back beside `setup`'s `CLAUDE.md` section, which is where the file those rules belong in is argued. |
| | Everyday git hygiene | ○ | ●●● | **13.** Worktrees are now taught in **two sections and named in two more** (`steering.worktree-each`, `goals.own-worktree`, plus a sentence each in `parallel` and `workshop`), and branch per task, small commits, and never letting an agent commit blind are still taught nowhere. The step has got better at the expensive half of this and has not started on the cheap half. |
...
- **Seventeen units hand the student something to do besides answer a question**: fifteen task
  ...
  **Only two units in the course carry nothing at all**, `recap` and `change management`, against
  ...
  third, step 0's `backend`, is empty by accident rather than by design (Table 1a).
...
- **The exercise heading is one key and the family has grown to fifteen units.** Every unit carrying
  ...
  `workshop`, which head their own sections. None of that is a defect.

### Sequence, in summary

- **Transitions stopped being the course's weakest habit in this range.** Of twenty-four seams
  between units, **nine are now marked from the side that leaves**: `welcome` → `backend`,
  `truth` → `workshop`, `recap` → step 2, `craft` → `steering`, `workflows` → `what it asks of you`,
  `what it asks of you` → `parallel workflows`, `parallel workflows` → `spending tokens`,
  `spending tokens` → `workshop` and `workshop` → `change management`. The last five of those are
  **consecutive**, so the course now runs unbroken from unit 18 to unit 23, across a module boundary
  in the middle of it. That is the first time this file has been able to write a sentence like that.
- **Where it is still bare is the front and the back.** Step 1's interior is seven consecutive
  ...
  seams are bare in both directions. And step 0 → step 1 was marked and is not any more, because
  `backend` now hands off to a page that does not exist (Table 1a).
